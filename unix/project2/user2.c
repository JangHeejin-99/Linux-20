...
void T_chat() {
	...
	key = ftok("shmfile", 1); // 서버와 같은 키 생성 
	shmid = shmget(key, 1024, 0); // 공유 메모리 정보 가져오기 

	shmaddr = (char*)shmat(shmid, NULL, 0); // 공유 메모리 연결 
	buf = shmaddr + 1; // 공유메모리[0]은 상태를 나타내는 flag, [1]부터 내용

	while(1) {
		if (shmaddr[0] == READ_R_FLAG) { // 사용자2가 공유 메모리에 작성하는 경우 
			printf("%s >> ", nick); // 자신의 아이디 출력 
			fgets(buf, 256, stdin); // 내용 입력 
			buf[strlen(buf)-1]='\0';
			if (strlen(buf) != 0) // 내용이 아무것도 없으면 다음 상태로 못넘어감 
				shmaddr[0] = PRINT_S_FLAG; // 사용자1이 공유 메모리에 있는 내용 출력 
		}
		if (shmaddr[0] == PRINT_R_FLAG) { // 사용자2가 공유 메모리에 있는 내용을 출력해야하는 경우 
			if (strlen(buf) != 0) { // 내용이 아무것도 없으면 다음 상태로 못넘어감 
				puts(buf); // 다른 사용자가 공유 메모리에 입력한 내용 출력 
				strcat(buf, "");
				shmaddr[0] = READ_R_FLAG; // 사용자2가 다시 공유 메모리에 작성
			}
		}
